...
export default function AdminFinancials() {
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      ...
      case 'pending':
        return 'bg-[#F1CB81]/20 text-[#3C291C] border border-[#F1CB81]/30';
      case 'failed':
        return 'bg-red-100 text-red-600 border border-red-200';
      default:
        return 'bg-[#3C291C]/5 text-[#3C291C]/60 border border-[#3C291C]/10';
    }
  };
  ...
  return (
    <div className="p-8 bg-[#FFFBF5] min-h-screen">
      <div className="max-w-7xl mx-auto">
        {/* Page Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-extrabold tracking-tight text-[#3C291C]">Financial Overview</h1>
          <p className="text-[#3C291C]/60 mt-1">Monitor platform revenue, commissions, and payout history.</p>
        </div>

        {/* Period Toggle */}
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3 bg-white rounded-xl border border-[#3C291C]/10 p-1">
            <button
              onClick={() => setSelectedPeriod('monthly')}
              className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${
                selectedPeriod === 'monthly'
                  ? 'bg-[#F1CB81] text-[#3C291C]'
                  : 'text-[#3C291C]/60 hover:text-[#3C291C]'
              }`}
            >
              Monthly
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-2xl border border-[#3C291C]/10 shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <MaterialIcon icon="monetization_on" className="text-[#F1CB81] text-3xl" />
              <span className="text-xs font-bold text-green-700 bg-green-100 px-2 py-1 rounded-full">
                +{metrics.revenueGrowth}%
              </span>
            </div>
            <p className="text-[#3C291C]/60 text-sm mb-1">Total Revenue (YTD)</p>
            <p className="text-2xl font-extrabold text-[#3C291C]">
          ...
          <div className="bg-white rounded-2xl border border-[#3C291C]/10 shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <MaterialIcon icon="account_balance_wallet" className="text-[#91ADCD] text-3xl" />
              <span className="text-xs font-bold text-green-700 bg-green-100 px-2 py-1 rounded-full">
                +{metrics.commissionGrowth}%
              </span>
            </div>
            <p className="text-[#3C291C]/60 text-sm mb-1">Total Commission</p>
            <p className="text-2xl font-extrabold text-[#3C291C]">
          ...
          <div className="bg-white rounded-2xl border border-[#3C291C]/10 shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <MaterialIcon icon="meeting_room" className="text-[#DB8B8C] text-3xl" />
              <span className="text-xs font-bold text-green-700 bg-green-100 px-2 py-1 rounded-full">
                +{metrics.studioGrowth}%
              </span>
            </div>
            <p className="text-[#3C291C]/60 text-sm mb-1">Active Studios</p>
            <p className="text-2xl font-extrabold text-[#3C291C]">{metrics.activeStudios}</p>
          </div>

          <div className="bg-white rounded-2xl border border-[#3C291C]/10 shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <MaterialIcon icon="payments" className="text-[#F1CB81] text-3xl" />
              <span className="text-xs font-bold text-red-600 bg-red-100 px-2 py-1 rounded-full">
                {metrics.pendingChange}%
              </span>
            </div>
            <p className="text-[#3C291C]/60 text-sm mb-1">Pending Payouts</p>
            <p className="text-2xl font-extrabold text-[#3C291C]">
        ...
        {/* Revenue Chart */}
        <div className="bg-white rounded-2xl border border-[#3C291C]/10 shadow-sm p-6 mb-8">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-extrabold text-[#3C291C]">Revenue Trends</h3>
            <div className="flex gap-4">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-[#F1CB81] rounded-full"></div>
                <span className="text-xs text-[#3C291C]/60">Revenue</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-[#91ADCD] rounded-full"></div>
                <span className="text-xs text-[#3C291C]/60">Commission</span>
              </div>
            </div>
          </div>

          <div className="relative h-80 w-full">
            <div className="absolute inset-0 flex flex-col justify-between text-xs text-[#3C291C]/40">
              <span>$350k</span>
              <span>$262k</span>
              <span>$175k</span>
              <span>$87k</span>
            ...
            <div className="ml-16 h-full relative">
              <div className="absolute inset-0 flex flex-col justify-between pointer-events-none">
                {[0, 1, 2, 3, 4].map((i) => (
                  <div key={i} className="border-t border-[#3C291C]/10 w-full"></div>
                ))}
              </div>
                ...
                {monthlyData.slice(0, 6).map((data, idx) => (
                  <div key={idx} className="flex-1 flex flex-col items-center gap-2">
                    <div className="relative w-full flex flex-col items-center gap-1">
                      <div
                        className="w-6 bg-[#F1CB81] rounded-t transition-all hover:bg-[#DB8B8C]"
                        style={{ height: `${(data.revenue / maxRevenue) * 100}%`, minHeight: '4px' }}
                      >
                        <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 text-[10px] text-[#3C291C] font-bold whitespace-nowrap">
                          ${(data.revenue / 1000).toFixed(0)}k
                        </div>
                      </div>
                      <div
                        className="w-6 bg-[#91ADCD] rounded-t transition-all"
                        style={{ height: `${(data.commission / maxRevenue) * 100}%`, minHeight: '4px' }}
                      ></div>
                    </div>
                    <span className="text-xs text-[#3C291C]/40">{data.month}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl border border-[#3C291C]/10 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-[#3C291C]/10 flex items-center justify-between">
            <h3 className="text-lg font-extrabold text-[#3C291C]">Payout History</h3>
            <button className="text-[#3C291C] text-sm font-bold hover:text-[#DB8B8C] flex items-center gap-1 transition-colors">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-[#3C291C]/5 text-xs font-bold text-[#3C291C]/40 uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-[#3C291C]/10">
                {paginatedPayouts.map((payout) => (
                  <tr key={payout.id} className="hover:bg-[#3C291C]/5 transition-colors">
                    <td className="px-6 py-4 text-sm text-[#3C291C]/60">{payout.date}</td>
                    <td className="px-6 py-4">
                      <div>
                        <p className="font-bold text-[#3C291C]">{payout.studioName}</p>
                        <p className="text-xs text-[#3C291C]/40">{payout.studioId}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm font-mono text-[#3C291C]/40">{payout.payoutId}</td>
                    <td className="px-6 py-4 text-sm font-bold text-[#3C291C]">${payout.amount.toLocaleString()}</td>
                    <td className="px-6 py-4 text-sm text-[#DB8B8C] font-medium">-${payout.commission.toLocaleString()}</td>
                    <td className="px-6 py-4 text-sm font-extrabold text-[#3C291C]">${payout.netAmount.toLocaleString()}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex px-3 py-1 rounded-full text-xs font-bold ${getStatusStyle(payout.status)}`}>
                        {payout.status.toUpperCase()}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <button className="p-2 text-[#3C291C]/40 hover:text-[#3C291C] hover:bg-[#3C291C]/5 rounded-lg transition-colors">
          ...
          {/* Pagination */}
          <div className="flex items-center justify-between px-6 py-4 border-t border-[#3C291C]/10 bg-[#3C291C]/5">
            <p className="text-xs text-[#3C291C]/40">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="p-2 rounded-lg border border-[#3C291C]/10 text-[#3C291C]/40 hover:text-[#3C291C] disabled:opacity-50 transition-colors"
              >

...
export default function FranchiseeLayout({
  ...
  const navItems = [
    { href: '/franchisee/dashboard', icon: <HomeIcon className="w-5 h-5" />, label: 'Dashboard' },
    { href: '/franchisee/studios', icon: <MaterialIcon icon="storefront" />, label: 'Studios' },
    { href: '/franchisee/revenue', icon: <CurrencyDollarIcon className="w-5 h-5" />, label: 'Revenue' },
    { href: '/franchisee/system-pulse', icon: <ChartBarIcon className="w-5 h-5" />, label: 'System Pulse' },
    { href: '/franchisee/network', icon: <MaterialIcon icon="hub" />, label: 'Network' },
    { href: '/franchisee/settings', icon: <Cog6ToothIcon className="w-5 h-5" />, label: 'Settings' },
  ];
          ...
          <div className={`p-6 ${isCollapsed ? 'px-4' : 'px-6'}`}>
              ...
              {!isCollapsed && (
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-primary rounded-lg flex items-center justify-center">
                    <MaterialIcon icon="domain" className="text-white text-xl" />
                  </div>
                  <div>
                    <h1 className="text-xl font-black tracking-tight">ManyRooms</h1>
                    <p className="text-[10px] text-primary font-bold tracking-widest">FRANCHISEE PORTAL</p>
                  </div>
                </div>
              )}
              {isCollapsed && (
            ...
            <div className="mt-auto pt-8">
              <Link
                ...
                title={isCollapsed ? 'New Studio' : ''}
              >
                <PlusCircleIcon className="w-5 h-5" />
                {!isCollapsed && <span>NEW STUDIO</span>}
              </Link>
            </div>
            ...
            <div className="flex items-center gap-4 flex-1">
              <div className="relative max-w-md w-full md:w-auto">
                ...
                <input
                  ...
                  placeholder="Search studios, analytics, or reports..."
                />
              </div>
            </div>
              ...
              <div className="flex items-center gap-2 md:gap-3">
                <div className="text-right hidden sm:block">
                  ...
                  <p className="text-xs md:text-sm font-medium truncate max-w-[100px] md:max-w-none">
                    {user?.user_metadata?.name || 'Franchisee Partner'}
                  </p>
                </div>
                <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-primary to-indigo-600 flex items-center justify-center text-white font-bold text-sm md:text-lg">
                  {user?.user_metadata?.name ? getInitials(user.user_metadata.name) : 'A'}
                </div>
              </div>
            </div>
          </header>

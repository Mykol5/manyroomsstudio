...
interface StudioFormData {
  name: string; category: string; capacity: number; description: string;
  street_address: string; city: string; state: string; postal_code: string; country: string;
  hourly_rate: number; daily_rate: number; weekly_rate: number; cleaning_fee: number;
  amenities: string[];
  availability: { monday: boolean; tuesday: boolean; wednesday: boolean; thursday: boolean; friday: boolean; saturday: boolean; sunday: boolean; startTime: string; endTime: string; };
  images: File[]; existingImages: string[]; removeImages: number[];
}
...
export default function EditStudioPage() {
  ...
  const [formData, setFormData] = useState<StudioFormData>({
    name: '', category: '', capacity: 1, description: '',
    street_address: '', city: '', state: '', postal_code: '', country: 'United States',
    hourly_rate: 0, daily_rate: 0, weekly_rate: 0, cleaning_fee: 0, amenities: [],
    availability: { monday: true, tuesday: true, wednesday: true, thursday: true, friday: true, saturday: true, sunday: true, startTime: '09:00', endTime: '22:00' },
    images: [], existingImages: [], removeImages: [],
  });
  ...
  const amenitiesList = ['Natural Light','Studio Lighting Kit','Backdrop Paper','Changing Room','Makeup Station','WiFi','AC/Heating','Parking','Kitchenette','Bluetooth Speakers','Projector','Whiteboard','Cyclorama Wall','Green Screen','Sound Treatment','Freight Elevator'];
  ...
  const fetchStudio = async () => {
    ...
    try {
      ...
      if (data.owner_id !== user?.id) { setError('You do not have permission to edit this studio'); return; }
      setFormData({
        name: data.name || '', category: data.category || '', capacity: data.capacity || 1,
        description: data.description || '', street_address: data.street_address || '',
        city: data.city || '', state: data.state || '', postal_code: data.postal_code || '',
        country: data.country || 'United States', hourly_rate: data.hourly_rate || 0,
        daily_rate: data.daily_rate || 0, weekly_rate: data.weekly_rate || 0, cleaning_fee: data.cleaning_fee || 0,
        amenities: data.amenities || [],
        availability: data.availability || { monday: true, tuesday: true, wednesday: true, thursday: true, friday: true, saturday: true, sunday: true, startTime: '09:00', endTime: '22:00' },
        images: [], existingImages: data.images || [], removeImages: [],
      });
    } catch (err: any) { setError(err.message || 'Failed to load studio'); }
    finally { setLoading(false); }
  };

  const handleInputChange = (field: keyof StudioFormData, value: any) => setFormData(prev => ({ ...prev, [field]: value }));
  const handleAmenityToggle = (amenity: string) => setFormData(prev => ({ ...prev, amenities: prev.amenities.includes(amenity) ? prev.amenities.filter(a => a !== amenity) : [...prev.amenities, amenity] }));
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files || []);
    if (formData.images.length + formData.existingImages.length + files.length > 10) { setError('Maximum 10 images allowed'); setTimeout(() => setError(''), 3000); return; }
    setFormData(prev => ({ ...prev, images: [...prev.images, ...files] })); setError('');
  };
  const removeExistingImage = (index: number) => setFormData(prev => ({ ...prev, removeImages: [...prev.removeImages, index], existingImages: prev.existingImages.filter((_, i) => i !== index) }));
  const removeNewImage = (index: number) => setFormData(prev => ({ ...prev, images: prev.images.filter((_, i) => i !== index) }));

  const handleSubmit = async () => {
    setSaving(true); setError('');
    try {
      const updateData: any = {
        name: formData.name, category: formData.category, capacity: formData.capacity,
        description: formData.description, street_address: formData.street_address,
        city: formData.city, state: formData.state, postal_code: formData.postal_code,
        country: formData.country, hourly_rate: formData.hourly_rate, daily_rate: formData.daily_rate,
        weekly_rate: formData.weekly_rate, cleaning_fee: formData.cleaning_fee,
        amenities: formData.amenities, availability: formData.availability, updated_at: new Date().toISOString(),
      };
      let updatedImages = [...formData.existingImages];
      for (const image of formData.images) {
        const bytes = await image.arrayBuffer();
        const buffer = Buffer.from(bytes);
        updatedImages.push(`data:${image.type};base64,${buffer.toString('base64')}`);
      }
      ...
      setTimeout(() => router.push(`/owner/studios/${studioId}`), 1500);
    } catch (err: any) { setError(err.message || 'Failed to update studio'); }
    ...
    return (
      <div className="min-h-screen bg-[#FFFBF5] flex items-center justify-center">
        <div className="animate-pulse space-y-4 text-center">
          ...
          <p className="text-[#3C291C] font-bold">Loading Studio...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#FFFBF5]">
      <div className="max-w-[1400px] mx-auto px-4 md:px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <Link href={`/owner/studios/${studioId}`} className="flex items-center gap-2 text-[#3C291C]/60 hover:text-[#3C291C] transition-colors font-bold text-sm mb-2">
              <ArrowLeftIcon className="w-4 h-4" /> Back to Studio Details
            </Link>
            <h1 className="text-3xl md:text-4xl font-extrabold text-[#3C291C] tracking-tight">Edit Studio</h1>
            <p className="text-[#3C291C]/60 text-sm mt-1">Update your studio information, photos, and pricing.</p>
          </div>
          <div className="flex gap-3">
            <button onClick={() => router.push(`/owner/studios/${studioId}`)}
              className="px-6 py-3 border-2 border-[#3C291C]/10 text-[#3C291C] rounded-xl font-bold text-sm hover:bg-[#3C291C]/5 transition-all">Cancel</button>
            <button onClick={handleSubmit} disabled={saving}
              className="flex items-center gap-2 px-6 py-3 bg-[#F1CB81] text-[#3C291C] rounded-xl font-bold text-sm hover:bg-[#DB8B8C] hover:text-white transition-all disabled:opacity-50 shadow-lg">
              <CheckCircleIcon className="w-5 h-5" /> {saving ? 'Saving...' : 'Save Changes'}
            </button>
          </div>
        </div>
          ...
          {sections.map((section) => (
            <button key={section.id} onClick={() => setActiveSection(section.id)}
              className={`flex items-center gap-2 px-5 py-3 rounded-xl font-bold text-sm whitespace-nowrap transition-all ${
                activeSection === section.id ? 'bg-[#F1CB81] text-[#3C291C]' : 'bg-white text-[#3C291C]/60 hover:bg-[#3C291C]/5 border border-[#3C291C]/10'
              }`}>
              <MaterialIcon icon={section.icon} className="text-lg" /> {section.label}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <textarea value={formData.description} onChange={(e) => handleInputChange('description', e.target.value)} rows={5}
                      placeholder="Describe your space, its unique features..."
              ...
              <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-[#3C291C]/10">
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <input type="text" value={formData.street_address} onChange={(e) => handleInputChange('street_address', e.target.value)}
                      placeholder="123 Creative Street"
                      className="w-full bg-[#3C291C]/5 border border-[#3C291C]/10 rounded-xl px-4 py-3 text-[#3C291C] focus:ring-2 focus:ring-[#F1CB81] outline-none placeholder:text-[#3C291C]/30" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="block text-sm font-bold text-[#3C291C] mb-2">City *</label><input type="text" value={formData.city} onChange={(e) => handleInputChange('city', e.target.value)} placeholder="New York" className="w-full bg-[#3C291C]/5 border border-[#3C291C]/10 rounded-xl px-4 py-3 text-[#3C291C] focus:ring-2 focus:ring-[#F1CB81] outline-none placeholder:text-[#3C291C]/30" /></div>
                    <div><label className="block text-sm font-bold text-[#3C291C] mb-2">State *</label><input type="text" value={formData.state} onChange={(e) => handleInputChange('state', e.target.value)} placeholder="NY" className="w-full bg-[#3C291C]/5 border border-[#3C291C]/10 rounded-xl px-4 py-3 text-[#3C291C] focus:ring-2 focus:ring-[#F1CB81] outline-none placeholder:text-[#3C291C]/30" /></div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="block text-sm font-bold text-[#3C291C] mb-2">Postal Code</label><input type="text" value={formData.postal_code} onChange={(e) => handleInputChange('postal_code', e.target.value)} placeholder="10001" className="w-full bg-[#3C291C]/5 border border-[#3C291C]/10 rounded-xl px-4 py-3 text-[#3C291C] focus:ring-2 focus:ring-[#F1CB81] outline-none placeholder:text-[#3C291C]/30" /></div>
                    <div><label className="block text-sm font-bold text-[#3C291C] mb-2">Country</label><input type="text" value={formData.country} onChange={(e) => handleInputChange('country', e.target.value)} placeholder="United States" className="w-full bg-[#3C291C]/5 border border-[#3C291C]/10 rounded-xl px-4 py-3 text-[#3C291C] focus:ring-2 focus:ring-[#F1CB81] outline-none placeholder:text-[#3C291C]/30" /></div>
                  </div>
                </div>
              </div>
            )}
            ...
            {activeSection === 'pricing' && (
              <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-[#3C291C]/10">
                ...
                <div className="space-y-5">
                  <div><label className="block text-sm font-bold text-[#3C291C] mb-2">Hourly Rate ($) *</label><input type="number" value={formData.hourly_rate} onChange={(e) => handleInputChange('hourly_rate', parseFloat(e.target.value) || 0)} placeholder="150" className="w-full bg-[#3C291C]/5 border border-[#3C291C]/10 rounded-xl px-4 py-3 text-[#3C291C] focus:ring-2 focus:ring-[#F1CB81] outline-none placeholder:text-[#3C291C]/30" /></div>
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="block text-sm font-bold text-[#3C291C] mb-2">Daily Rate ($)</label><input type="number" value={formData.daily_rate} onChange={(e) => handleInputChange('daily_rate', parseFloat(e.target.value) || 0)} placeholder="800" className="w-full bg-[#3C291C]/5 border border-[#3C291C]/10 rounded-xl px-4 py-3 text-[#3C291C] focus:ring-2 focus:ring-[#F1CB81] outline-none placeholder:text-[#3C291C]/30" /></div>
                    <div><label className="block text-sm font-bold text-[#3C291C] mb-2">Weekly Rate ($)</label><input type="number" value={formData.weekly_rate} onChange={(e) => handleInputChange('weekly_rate', parseFloat(e.target.value) || 0)} placeholder="4000" className="w-full bg-[#3C291C]/5 border border-[#3C291C]/10 rounded-xl px-4 py-3 text-[#3C291C] focus:ring-2 focus:ring-[#F1CB81] outline-none placeholder:text-[#3C291C]/30" /></div>
                  </div>
                  <div><label className="block text-sm font-bold text-[#3C291C] mb-2">Cleaning Fee ($)</label><input type="number" value={formData.cleaning_fee} onChange={(e) => handleInputChange('cleaning_fee', parseFloat(e.target.value) || 0)} placeholder="50" className="w-full bg-[#3C291C]/5 border border-[#3C291C]/10 rounded-xl px-4 py-3 text-[#3C291C] focus:ring-2 focus:ring-[#F1CB81] outline-none placeholder:text-[#3C291C]/30" /></div>
                </div>
              </div>
            )}
              ...
              <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-[#3C291C]/10">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {amenitiesList.map((amenity) => (
                    <label key={amenity} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all border-2 ${formData.amenities.includes(amenity) ? 'bg-[#F1CB81]/20 border-[#F1CB81] text-[#3C291C]' : 'bg-[#3C291C]/5 border-transparent text-[#3C291C]/60 hover:bg-[#3C291C]/10'}`}>
                      <input type="checkbox" checked={formData.amenities.includes(amenity)} onChange={() => handleAmenityToggle(amenity)} className="sr-only" />
                      <span className={`text-lg ${formData.amenities.includes(amenity) ? 'text-[#3C291C]' : 'text-[#3C291C]/30'}`}>{formData.amenities.includes(amenity) ? '✓' : '○'}</span>
            ...
            {activeSection === 'availability' && (
              <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-[#3C291C]/10">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      {['monday','tuesday','wednesday','thursday','friday','saturday','sunday'].map((day) => (
                        <label key={day} className={`flex items-center justify-center p-3 rounded-xl cursor-pointer transition-all border-2 ${(formData.availability as any)[day] ? 'bg-[#F1CB81]/20 border-[#F1CB81] text-[#3C291C] font-bold' : 'bg-[#3C291C]/5 border-transparent text-[#3C291C]/60 hover:bg-[#3C291C]/10'}`}>
                          <input type="checkbox" checked={(formData.availability as any)[day]} onChange={(e) => handleInputChange('availability', { ...formData.availability, [day]: e.target.checked })} className="sr-only" />
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="block text-sm font-bold text-[#3C291C] mb-2">Start Time</label><input type="time" value={formData.availability.startTime} onChange={(e) => handleInputChange('availability', { ...formData.availability, startTime: e.target.value })} className="w-full bg-[#3C291C]/5 border border-[#3C291C]/10 rounded-xl px-4 py-3 text-[#3C291C] focus:ring-2 focus:ring-[#F1CB81] outline-none" /></div>
                    <div><label className="block text-sm font-bold text-[#3C291C] mb-2">End Time</label><input type="time" value={formData.availability.endTime} onChange={(e) => handleInputChange('availability', { ...formData.availability, endTime: e.target.value })} className="w-full bg-[#3C291C]/5 border border-[#3C291C]/10 rounded-xl px-4 py-3 text-[#3C291C] focus:ring-2 focus:ring-[#F1CB81] outline-none" /></div>
                  </div>
                </div>
              </div>
            )}
            ...
            {activeSection === 'photos' && (
              <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-[#3C291C]/10">
                ...
                {formData.existingImages.length > 0 && (
                  <div className="mb-6">
                    <p className="text-sm font-bold text-[#3C291C] mb-3">Current Photos</p>
                    <div className="grid grid-cols-3 md:grid-cols-4 gap-3">
                      {formData.existingImages.map((img, idx) => (
                        <div key={idx} className="relative aspect-square rounded-xl overflow-hidden bg-[#3C291C]/5 group">
                          <img src={img} alt={`Studio ${idx + 1}`} className="w-full h-full object-cover" />
                          <button type="button" onClick={() => removeExistingImage(idx)} className="absolute top-2 right-2 p-1.5 bg-red-500 rounded-lg text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600"><TrashIcon className="w-4 h-4" /></button>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {formData.images.length > 0 && (
                  <div className="mb-6">
                    <p className="text-sm font-bold text-[#3C291C] mb-3">New Photos</p>
                    <div className="grid grid-cols-3 md:grid-cols-4 gap-3">
                      {formData.images.map((img, idx) => (
                        <div key={idx} className="relative aspect-square rounded-xl overflow-hidden bg-[#3C291C]/5 group">
                          <img src={URL.createObjectURL(img)} alt={`New ${idx + 1}`} className="w-full h-full object-cover" />
                          <button type="button" onClick={() => removeNewImage(idx)} className="absolute top-2 right-2 p-1.5 bg-red-500 rounded-lg text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600"><TrashIcon className="w-4 h-4" /></button>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {(formData.existingImages.length + formData.images.length) < 10 && (
                  <button type="button" onClick={() => fileInputRef.current?.click()}
                    ...
                    <span className="text-xs text-[#3C291C]/40">Maximum 10 photos</span>
                  </button>
                )}
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#3C291C]/10">
              <h3 className="text-lg font-extrabold text-[#3C291C] mb-4">Preview</h3>
              <div className="space-y-3 text-sm">
                {[['Name', formData.name], ['Category', formData.category], ['Capacity', `${formData.capacity} people`], ['Rate', `$${formData.hourly_rate}/hr`], ['Location', `${formData.city || '—'}, ${formData.state || '—'}`], ['Photos', `${formData.existingImages.length + formData.images.length}`]].map(([label, value]) => (
            ...
            <div className="bg-[#91ADCD]/20 border border-[#91ADCD]/30 rounded-2xl p-6">
              <h3 className="text-sm font-extrabold text-[#3C291C] mb-2">Review Required</h3>
              <p className="text-sm text-[#3C291C]/60">Changes to your studio may require a new review by our team before appearing in search results.</p>
            </div>

            <button onClick={handleSubmit} disabled={saving}
              className="w-full py-4 bg-[#F1CB81] text-[#3C291C] rounded-2xl font-extrabold text-lg hover:bg-[#DB8B8C] hover:text-white transition-all disabled:opacity-50 shadow-lg">

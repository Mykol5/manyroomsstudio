...
(function ($) {
	...
	$('.tp-contact-me-interest-category .wpcf7-checkbox .wpcf7-list-item:nth-child(3) label input').prop('checked', true);
	...
	// 04. Masonary Js//
	// Isotope `.grid` + `.masonary-menu` filter init moved to per-widget file:
	//   pixora-core/include/elementor/pixora/px-portfolio-filter/px-portfolio-filter-init.js
	// registered as `pixora-el-pixora-px-portfolio-filter`.
	// Was duplicated in main.js AND main-px.js — both fired on every page (A7 #11).

	// 05. magnificPopup img/video //
	// `.popup-image` and `.popup-video` magnificPopup inits moved to per-widget files:
	//   tab/tab, px-portfolio/portfolio-slicer (popup-image)
	//   video/video-popup, rating/rating-info, blog/blog-grid (popup-video)
	// Was duplicated in main.js AND main-px.js — both copies firing on every page (A7 #11).

	// 07. Counter Js //
	// `new PureCounter()` (`.purecounter`) moved to per-widget init files for:
	//   counter/counter, pixora/px-counter-box, testimonial/testimonial-featured,
	//   skill/skillbar, hero/hero-banner-2.
	// The `.filesizecount` variant init was removed — no active widget emits that class.
	...
	// hoverEffect (.tp--hover-item / .tp--hover-img / data-displacement) WebGL
	// init moved to per-widget shared init at
	// pixora-core/include/elementor/image/image-hover/image-hover-init.js
	// registered as `pixora-el-image-image-hover` and depended on by 4 widgets:
	// image-hover, about-image, team-box, project-box.
	...
	// addClass removeClass
	$('.tp-offcanvas-2-area .tp-offcanvas-menu > nav > ul > li').addClass('is-active');
	...
	// addClass removeClass-2
	function designWorkAnims() {
	...
	// Ensure GSAP and SplitText are loaded
	gsap.matchMedia().add("(min-width: 991px)", () => {
		document.querySelectorAll(".title-box").forEach((box) => {
			...
			if (rightElements.length) {
				tl.fromTo(rightElements, { xPercent: themeDirection == 'rtl' ? -50 : 50 }, { xPercent: themeDirection == 'rtl' ? 20 : -20, ease: "power1.out" }, 0);
			}
		});
	});
	...
	if (document.querySelector('.design-project-item')) {
		...
		pw.add("(min-width: 991px)", () => {
			...
			const x1 = isRTL ? -400 : 400;
			const x2 = isRTL ? 400 : -400;
			const rotate1 = isRTL ? -10 : 10;
			const rotate2 = isRTL ? 10 : -10;

			gsap.set('.design-project-thumb.item-1', { x: x1, rotate: rotate1 });
			gsap.set('.design-project-thumb.item-2', { x: x2, rotate: rotate2 });
	...
	if (document.querySelectorAll('.design-choose-item-wrap')) {
		...
		pw.add("(min-width: 1200px)", () => {
			...
			const x1 = isRTL ? 400 : -400;
			const x2 = isRTL ? -400 : 400;
			const rotate1 = isRTL ? 40 : -40;
			const rotate2 = isRTL ? -40 : 40;

			document.querySelectorAll('.design-choose-item-wrap').forEach(item => {
				gsap.set(item.querySelector('.design-choose-item-1'), { x: x1, rotate: rotate1 });
				gsap.set(item.querySelector('.design-choose-item-2'), { x: x2, rotate: rotate2 });
	...
	// AtroposComponent removed — no active widget renders <atropos-component>.
	// atropos.js is now register-only in pixora-scripts.php.
	...
	/////////////////////////////////
	// Rendom text genarator in hover
	...
	// Section to title zoom and item uper
	...
	var contactCategoryBtns = $('.tp-contact-category-btn');
	...
	// 03. Nice Select Js//
	$('.tp-select select, .sidebar-widget select, .tp-footer-widget select, .postbox-wrapper select').niceSelect();


	// last child menu
	$('.tp-pixora-default-header nav > ul > li').slice(-4).addClass('menu-last');

	///////////////////////////////
	// popup subscribe js
	if (document.querySelector('.subscribe-popup')) {
		const loginPopup = document.querySelector(".subscribe-popup");
		const close = document.querySelector(".close"); // Ensure this selector is correct
		...
		function showPopup() {
			const timeLimit = parseInt(loginPopup.getAttribute('data-timelimit'), 10); // Correctly fetch and parse
			let i = 0;
	...
	// wooRelatedProducts
	var wooRelatedProducts = new Swiper('.tp-woo-related-product-active', {
		loop: true,
		slidesPerView: 3,
		spaceBetween: 30,
		speed: 1000,
	...
	windowOn.on("elementor/frontend/init", function () {
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-header-01.default", tp_mega_menu
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-header-03.default", tp_mega_menu
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-header-04.default", tp_mega_menu
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-header-06.default", tp_mega_menu
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-header-08.default", tp_mega_menu
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-header-09.default", tp_mega_menu
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-header-10.default", tp_mega_menu
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-header-11.default", tp_mega_menu
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-header-12.default", tp_mega_menu
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-header-14.default", tp_mega_menu
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-header-15.default", tp_mega_menu
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-text-scroll.default", moving_text
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-text-scroll.default", moving_text
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-project-scroll-list.default", designWorkAnim
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-award.default", designWorkAnims
		);
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-image-scroll.default", moving_text
		);
	});
